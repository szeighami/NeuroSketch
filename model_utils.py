...
def call_tree(config, DB, queries, test_queries, res, test_res):
    ...
    my_model, _, processes = build_tree(config['depth'], config['no_filters'], config['in_dim'], queries, in_res, test_queries, in_test_res, processes, config['exp_name'], "", 0, 0, config['no_processes'], config['path_to_neurosketch'])
...
def test_model(config, test_qs, test_res):
    ...
    c_call = config["path_to_neurosketch"]+"/run_model "+str(config['no_filters'])+" " + str(config['in_dim'])+" " + str(config['out_dim']) +' ' + str(config['depth'])+ ' ' + config['exp_name'] + ' ' + config['exp_name'] + " 10000 1"
    print("Testing model")
    os.system(c_call)
